...
void ViewManager::ProcessKeyboardEvents()
{
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_P) == GLFW_PRESS)
	{
		...
		// update camera for perspective mode
		g_pCamera->Position = glm::vec3(0.0f, 5.0f, 12.0f);
		g_pCamera->Front = glm::vec3(0.0f, -0.5f, -2.0f);
	}
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS)
	{
		...
		// set orthographic camera directly in front of the object
	...
	{
		g_pCamera->ProcessKeyboard(UP, gDeltaTime);         //setting q to up and e to down
	}
...
void ViewManager::PrepareSceneView()
{
	...
	if (!bOrthographicProjection)
	{
		projection = glm::perspective(
			...
			0.3f, 100.0f);   //editing this to increase view distance, was janking up the scene.
	}
	else
	{
